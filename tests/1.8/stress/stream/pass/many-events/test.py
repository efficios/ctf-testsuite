...
def generate_metadata(array_len):
	...
	for i in range(array_len):
		metadata_f.write(metadata_event.format(i, i))
	metadata_f.close()
...
def generate_stream(array_len):
	...
	# generate 8 bytes per event
	for i in range(array_len):
		write_binary(stream_f, struct.pack('<Q', i))
	stream_f.close()

def test_prepare():
	print('Preparing test for ' + str(array_len) + ' events')
	os.mkdir(tracedir_name)
	generate_metadata(array_len)
	generate_stream(array_len)

def test_clean():
	print('Cleaning up test for ' + str(array_len) + ' events')
	try:
